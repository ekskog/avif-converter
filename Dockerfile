...
RUN git clone https://github.com/strukturag/libde265.git && \
  cd libde265 && ./autogen.sh && ./configure --prefix=/usr && \
...
# === Runtime stage: Minimal image with node and sharp ===
FROM node:18-alpine

WORKDIR /app

# Install runtime dependencies needed for libvips
RUN apk add --no-cache \
